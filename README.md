...
На macOS перед добавлением split routes клиент закрепляет внешние адреса отдельными статическими host routes. Кэшированная запись `HOST,WASCLONED` сама по себе для этого не годится. Существующие глобальные статические host routes и записи непосредственно подключённых соседей сохраняются; при остановке удаляются только успешно добавленные клиентом маршруты. После настройки клиент повторно проверяет интерфейс и шлюз каждого внешнего адреса и выводит `underlay verified: ...`. Если маршрут ушёл в туннель или изменился, настройка завершается ошибкой и откатывается. При SIP-таймауте сначала проверь эти строки: `route -n get <IP-сервера>` должен показывать исходную сеть, а не созданный utun.
