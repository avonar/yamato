...
При Ctrl+C журнал показывает `shutdown requested`, `TUN closed`, каждую команду отката с префиксом `network restore:` и, после завершения, `shutdown complete`. Если удалить правило или восстановить настройку не удалось, выводится конкретная ошибка, процесс завершается с ненулевым кодом и не пытается переподключиться поверх неполностью восстановленной сети. Уже запущенной команде настройки даётся завершиться (таймаут 10 секунд), затем выполняется её откат; новые изменения после отмены не добавляются. Команды отката выполняются в отдельной группе процессов, чтобы повторный Ctrl+C не прерывал их посередине.
